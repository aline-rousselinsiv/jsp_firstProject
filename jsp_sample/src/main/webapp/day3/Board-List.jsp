...
<body>
	...
	<div>
		<select id="number" onchange="fnNumber(this.value)"> <!-- sending our option to the function -->
			<%
		...
		<table>
			<tr>
				...
				<%
					...
					if(orderKind == null){
				%>
					<a href="javascript:;" onclick="fnList('CNT', 'DESC')">조회수</a></th>
				<% 	
					} else if(orderKind.equals("DESC")){
				%>
					<a href="javascript:;" onclick="fnList('CNT', 'ASC')">조회수🔻</a></th>
				<%		
					} else {
				%>
					<a href="javascript:;" onclick="fnList('CNT', 'DESC')">조회수🔺</a></th>
			...
			<%
				...
				/* 페이징 변수*/
				int pageSize = 5; // 한 페이지에 몇개씨 보여줄 지
				int currentPage = 1;
				...
				/* 현재 페이지 위치에 따라서 자져올 값을 정하기 위해 offset 구하기 */
